...
# installp("VennDiagram")

library(VennDiagram)

if ("pdf" == "png") {
	png(filename="file.k27_ctcf.vennDiagram.png", width=10, height=10,
	res=300, units="cm")
} else if ("pdf" == "eps") {
	postscript(file="file.k27_ctcf.vennDiagram.eps", onefile=FALSE, horizontal=FALSE, 
	paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
	pdf(file="file.k27_ctcf.vennDiagram.pdf", onefile=FALSE, 
	paper="special")
}else if ("pdf" == "svg") {
	svg(filename="file.k27_ctcf.vennDiagram.svg", width=10, height=10,
	pointsize=10)
} else {
...
if (! FALSE) {
	...
	if("h3k27ac" != "CHENTONG"){
		h3k27ac <- data[grepl("\\<h3k27ac\\>",data[,2]),1]
		num <- num + 1
	}

	if("ctcf" != "CHENTONG"){
		ctcf <- data[grepl("\\<ctcf\\>",data[,2]),1]
		num <- num + 1
	}
	...
	if(num == 5){
		p <- venn.diagram( 
			x = list(h3k27ac=h3k27ac, CHENTONG=CHENTONG,
			CHENTONG=CHENTONG, ctcf=ctcf,
			CHENTONG=CHENTONG),
		...
		p <- venn.diagram( 
			x = list(h3k27ac=h3k27ac, CHENTONG=CHENTONG, ctcf=ctcf,
			CHENTONG=CHENTONG),
			filename = NULL, col = "black", lwd = 1, 
			fill = color_v,
			alpha = 0.50, main="", 
		...
		p <- venn.diagram( 
			x = list(h3k27ac=h3k27ac, ctcf=ctcf, CHENTONG=CHENTONG),
		...
		p <- venn.diagram( 
			x = list(h3k27ac=h3k27ac, ctcf=ctcf),
			filename = NULL, col = "transparent", 
			fill = color_v,
			alpha = 0.50, main="", 
			label.col = c("black"),
...
#ggsave(p, filename="file.k27_ctcf.vennDiagram.pdf", dpi=300, width=10,
