...
***MODIFIED BY Lin Dechun (lindechun@genomics.cn)***
...
<aes>
    ...
    -c  Name for color and fill variable column.
    ...
    -l  Levels for color and fill variable
        [${txtred}Default data order,accept a string like
...
<scales>
    ...
    -S  A number to add y axis（to -d)
        ...
        TRUE.${txtrst}]
    -C  Manually set colors for each box. Default FALSE, meaning using ggplot2 default Color for each bar.
...
<<outlier>>
    ...
    -O  The scales for you want to zoom in to exclude outliers.
        [${txtred}Default 1.05. Not recommend to change unless you know
...
## alpha_variable 的功能还没加，sp_boxplot_being.sh有一部分修改的记录，但是要参考scatterplot2的平均值求法，更合理。
...
while getopts "hf:m:Q:z:q:H:a:d:c:L:l:W:k:t:x:y:b:s:S:W:C:G:g:M:R:w:u:P:r:E:T:p:e:o:X:Y:n:K:v:V:j:J:A:D:B:i:O" OPTION
do
    case $OPTION in
        h)
            usage
...
if test "$melted" == "FALSE"; then
    # echo " the data file(-f) you give is unmelted, but there will have variable and value columns in dataFrame after melted"
    if [ -z $xvariable ]; then
        xvariable="variable"
...
### sort by group
if test "$sort_by_group" != "FALSE" && test "$sort_var" != "FALSE" && test "$color_variable" == "";then
    echo " You set -X"${sort_var}", but you don't set -c(color/fill variable), so we ignore -Y TRUE"
    sort_by_group='FALSE'
fi
######################### sort value ################################
...
s_boxplot <- function(dat,group_va){
    ...
    median_dat <- dat %>% group_by_(.dots=group_va) %>% summarise(median=median(${value}))
    return(median_dat)
...
if ("$color_v" != "FALSE") {
    if(${violin_jitter} || ${jitter}){
        p <- p +scale_colour_manual(values=c(${color_v}))
    }else{
